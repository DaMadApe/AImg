...
class TimbiricheNet(nn.Module):
    ...
    def __init__(self, game, args):
        ...
        # self.p_head = nn.Sequential( 
        #     nn.Linear(units, self.action_size),
        #     nn.Softmax()) 
...
class ModeloNeuronal(NeuralNet):
    ...
    def train(self, examples):
        ...
        """
        # Configura los módulos de la red en modo entrenamiento
        # Activa Dropouts, y BatchNorm reevalúa parámetros estadísticos
        self.nnet.train()

        # Se usa Adam por su capacidad de convergencia y
        # su permisividad con los hiperparámetros
        optimizer = optim.Adam(self.nnet.parameters())
        for epoch in range(args.epochs):
            print(f'--- Época: {epoch+1} ---')

            # Herramienta del repo para ver progreso de entrenamiento
            ...
            for _ in t:
                ...
                loss_pi = self.loss_pi(out_pi, target_pis)
                loss_v = self.loss_v(out_v, target_vs)

                # Alternativas de pérdida por experimentar
    ...
    # self.loss_pi y self.loss_v son las pérdidas citadas en el paper
